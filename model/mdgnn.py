...
class MDGNN(torch.nn.Module):
  def __init__(self, neighbor_finder, node_features, edge_features, device, n_layers=2,
               n_heads=2, dropout=0.1, use_memory=False, use_pres=False,
               memory_update_at_start=True, message_dimension=100,
               memory_dimension=500, embedding_module_type="graph_sum", #jw: change from attention to graph sum
    ...
    self.n_nodes = self.node_raw_features.shape[0]  #number of total nodes
    ...
    if self.use_pres:
      ...
      self.filter = Filter(n_nodes=self.n_nodes,
  ...
  def compute_temporal_embeddings(self, source_nodes, destination_nodes, negative_nodes, edge_times,
    ...
    if self.use_memory:
      ...
      #PRES: combine with the filter
      if self.use_pres:
        self.prev_memory = self.memory.get_memory(list(range(self.n_nodes))).clone()
    ...
    #determine if to use filter
    if self.use_pres:
        transition_dense = self.filter.get_incretment(list(range(self.n_nodes)))
        ...
        memory = self.gamma * pred_memory + (1- self.gamma) * memory
        ...
        if not (change.max().item() == 0):     
          ...
          change /=change.max() 
          change = 2*change - 1
          self.filter.update(list(range(self.n_nodes)),change)
